...
import { SAMPLE_GROUP_1_JSON, SAMPLE_GROUP_2_JSON } from "../constants/samples";
...
function getMethodPillClass(entry: PermissionEntry): string {
...
const PermissionListSection: FC<PermissionListSectionProps> = ({
	...
	const boxClass =
		"border-slate-300 bg-white/90 shadow-sm dark:border-slate-700/70 dark:bg-slate-900/70 dark:shadow-none";
			...
			<div className="flex items-center justify-between gap-3 border-b border-inherit px-4 py-3">
				...
				<div className="flex items-center gap-2">
					<button
						...
						className="rounded-md border border-slate-300 px-2 py-1 text-xs transition hover:bg-slate-100 dark:border-slate-500/50 dark:hover:bg-slate-800/30"
					>
						Copy JSON
					</button>
					<button
						...
						<>
							...
							<div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
								<div className={`rounded-xl border p-3 ${panelClasses}`}>
									<h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{result.group1.label} role breakdown</h4>
								...
								<div className={`rounded-xl border p-3 ${panelClasses}`}>
									<h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{result.group2.label} role breakdown</h4>
									<ul className="mt-2 space-y-1 text-sm">
