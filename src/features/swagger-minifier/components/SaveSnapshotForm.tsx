		...
		>
			<div className="flex flex-wrap items-end gap-2">
				<label className="flex min-w-[200px] flex-1 flex-col gap-1">
					...
					<div className="flex min-h-[2.125rem] min-w-0 overflow-hidden rounded-md border border-slate-300 bg-white transition focus-within:border-accent dark:border-slate-700 dark:bg-slate-950/90">
						<input
							type="text"
							{...form.register("name")}
			...
			{form.formState.errors.name?.message ? (
				<p className="text-xs text-amber-700 dark:text-amber-300">{form.formState.errors.name.message}</p>
			) : null}
			{submitError ? <p className="text-xs text-amber-700 dark:text-amber-300">{submitError}</p> : null}
		</form>
	);
};
